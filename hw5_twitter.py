...
import json
import requests
import secrets1 # file that contains your OAuth credentials

CACHE_FILENAME = "twitter_cache.json"
CACHE_DICT = {}

client_key = secrets1.TWITTER_API_KEY
client_secret = secrets1.TWITTER_API_SECRET
access_token = secrets1.TWITTER_ACCESS_TOKEN
access_token_secret = secrets1.TWITTER_ACCESS_TOKEN_SECRET
...
def make_request_with_cache(baseurl, hashtag, count):
    ...
    dict
        ...
        JSON
    '''
    request_key = construct_unique_key(baseurl, {'q': hashtag, 'count': count})
    if request_key in CACHE_DICT.keys():
        print("cache hit!", request_key)
        return CACHE_DICT[request_key]
    else:
        print("cache miss!", request_key)
        CACHE_DICT[request_key] = make_request(baseurl, {'q': hashtag, 'count': count})
...
def find_most_common_cooccurring_hashtag(tweet_data, hashtag_to_ignore):
    ...
    for tweet in tweet_data['statuses']:
        ...
        for h in hash_list:
            h_name = h['text'].lower()
            if h_name in h_dictionary.keys():
                h_dictionary[h_name] += 1
            else: 
                h_dictionary[h_name] = 1
    for h in h_dictionary.keys():
        hash_with_pound = "#" + h
        if hash_with_pound != hashtag_to_ignore and h_dictionary[h] > max_count:
            max_count = h_dictionary[h]
            most_co = h
    return "#" +  most_co
